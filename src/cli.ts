  ...
  .action(async (input: string, options: any) => {
    try {
      ...
      const generator = new TypeScriptGenerator({
        ...
        excludeOperationIds: options.excludeOps ? options.excludeOps.split(/[,\s]+/).map((s: string) => s.trim().toLowerCase()).filter(Boolean) : (generatorConfig.permissions?.block?.operationIds?.map((s: string) => s.trim().toLowerCase()) || undefined),
